...
3. 配置主节点的浮动 IP: 192.168.3.200/24
...
3. 创建自动故障转移时的回调脚本`/usr/local/bin/master_ip_failover`：
...
4. 创建 MYSQL 集群的配置文件：`/etc/masterha/app1.cnf`
...
1. 在 windows 主机通过浮动 IP 登录并查看 server_id:
...
3. 再次在 windows 主机通过浮动 IP 登录并查看 server_id:
...
2. Client 端使用 10 条线程插入 10w 条数据，插入采用重试策略，插入返回失败会继续重试到成功为止。
...
3. 开始插入前MYSQL 集群处于正常状态：主节点（node1），从节点（node2，node3），MHA Manager 正常监控。

4. 在插入的过程中关闭主节点的 MYSQL 实例，此时 Client 端会插入失败，并且不断重试。
   ![图 2](mha_failover_2_1.png)

5. 验证：MHA 故障转移成功，Client 重试成功，继续完成后面的数据插入。
   ![图 3](mha_failover_2_2.png)

   查询 node2 和 node3 的数据完整性
...
4. 修改 `/etc/masterha/app1.cnf` 的 `secondary_check_script` 参数
